...
tic;
[bV, bU, blockInd, trialCnt, snap] = blockSVD(opts); %this loads raw data and does the first blockwise SVD
toc;
...
[nU, s, nV] = fsvd(bV); %combine all blocks in a second SVD
...
% reduce to dimCnt components
U = U(:, 1:opts.dimCnt); %this is pixels x components
newV = nV(1:opts.dimCnt,:); %this is components x frames

%% do hemodynamic correction
corrV = reshape(newV, size(newV,1), [], 2); % split channels
U = reshape(U,size(snap,1),size(snap,2),[]); %reshape to frame format

% do hemodynamic correction
[Vc, regC, T, hemoVar] = Widefield_SvdHemoCorrect(U, corrV(:,:,1), corrV(:,:,2), opts.frameRate);
